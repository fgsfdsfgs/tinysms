...
void
OPLLDump2Patch (const uint8_t * dump, OPLL_PATCH * patch)
{
...
void
OPLLGetDefaultPatch (int32_t type, int32_t num, OPLL_PATCH * patch)
{
  OPLLDump2Patch (default_inst[type] + num * 8, patch);
}
...
makeDefaultPatch ()
{
  ...
  for (i = 0; i < OPLL_TONE_NUM; i++)
    for (j = 0; j < 19; j++)
      OPLLGetDefaultPatch (i, j, &default_patch[i][j * 2]);

}

void
OPLLSetPatch (OPLL * opll, const uint8_t * dump)
{
  OPLL_PATCH patch[2];
  int i;
...
void
OPLLPatch2Dump (const OPLL_PATCH * patch, uint8_t * dump)
{
...
void
OPLLCopyPatch (OPLL * opll, int32_t num, OPLL_PATCH * patch)
{
...
static void
OPLLSlotReset (OPLL_SLOT * slot, int type)
{
  slot->type = type;
...
OPLL *
OPLLInit (uint32_t clk, uint32_t rate)
{
  OPLL *opll;
  int32_t i;
  ...
  for (i = 0; i < 14; i++)
    opll->pan[i][0] = opll->pan[i][1] = 1.f;

  opll->mask = 0;

  OPLLReset (opll);
  OPLLResetPatch (opll, 0);
...
void
OPLLResetPatch (OPLL * opll, int32_t type)
{
  int32_t i;

  for (i = 0; i < 19 * 2; i++)
    OPLLCopyPatch (opll, i, &default_patch[type % OPLL_TONE_NUM][i]);
}
...
void
OPLLSetRate (OPLL * opll, uint32_t r)
{
  if (opll->quality)
    rate = 49716;
  else
...
void
OPLLSetQuality (OPLL * opll, uint32_t q)
{
  opll->quality = q;
  OPLLSetRate (opll, rate);
}
...
void
OPLLWriteReg (OPLL * opll, uint32_t reg, uint32_t data)
{
  int32_t i, v, ch;
...
void
OPLLWriteIO (OPLL * opll, uint32_t adr, uint32_t val)
{
  if (adr & 1)
    OPLLWriteReg (opll, opll->adr, val);
  else
    opll->adr = val;
}
...
/* STEREO MODE (OPT) */
void
OPLLSetPan (OPLL * opll, uint32_t ch, int32_t pan)
{
	uint32_t fnl_ch;
...
static void
calcStereo (OPLL * opll, int32_t out[2])
{
...
/*  int32_t r[4] = { 0, 0, 0, 0 };        /* Ignore, Right, Left, Center */
  ...
  for (i = 0; i < 6; i++)
    if (!(opll->mask & OPLL_MASK_CH (i)) && (CAR(opll,i)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[i][0] );
        r += (int32_t)( channel * opll->pan[i][1] );
      }
    }
 ...
 if (! opll->vrc7_mode)
 {
  if (opll->patch_number[6] <= 15)
  {
    if (!(opll->mask & OPLL_MASK_CH (6)) && (CAR(opll,6)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[6][0] );
        r += (int32_t)( channel * opll->pan[6][1] );
      }
  ...
  else
  {
    if (!(opll->mask & OPLL_MASK_BD) && (CAR(opll,6)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[9][0] );
        r += (int32_t)( channel * opll->pan[9][1] );
      }
    }
  }
  ...
  if (opll->patch_number[7] <= 15)
  {
    if (!(opll->mask & OPLL_MASK_CH (7)) && (CAR (opll,7)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[7][0] );
        r += (int32_t)( channel * opll->pan[7][1] );
      }
    }
  }
  else
  {
    if (!(opll->mask & OPLL_MASK_HH) && (MOD (opll,7)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[10][0] );
        r += (int32_t)( channel * opll->pan[10][1] );
      }
    }
    if (!(opll->mask & OPLL_MASK_SD) && (CAR (opll,7)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[11][0] );
        r += (int32_t)( channel * opll->pan[11][1] );
      }
    }
  }
  ...
  if (opll->patch_number[8] <= 15)
  {
    if (!(opll->mask & OPLL_MASK_CH (8)) && (CAR (opll,8)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[8][0] );
        r += (int32_t)( channel * opll->pan[8][1] );
      }
    }
  }
  else
  {
    if (!(opll->mask & OPLL_MASK_TOM) && (MOD (opll,8)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[12][0] );
        r += (int32_t)( channel * opll->pan[12][1] );
      }
    }
    if (!(opll->mask & OPLL_MASK_CYM) && (CAR (opll,8)->eg_mode != FINISH))
    {
      ...
      {
        l += (int32_t)( channel * opll->pan[13][0] );
        r += (int32_t)( channel * opll->pan[13][1] );
      }
    }
  }
 } // end if (! opll->vrc7_mode)
...
/*void
OPLL_calc_stereo (OPLL * opll, int32_t out[2], samples)
{
  if (!opll->quality)
  {
...
void
OPLLCalcStereo (OPLL * opll, int32_t **out, int32_t samples)
{
  int32_t *bufMO = out[0];
  int32_t *bufRO = out[1];
  int32_t buffers[2];
  ...
  for( i=0; i < samples ; i++ )
  {
    ...
    else
    {
      ...
      { 
        ...
        opll->sprev[0] = opll->snext[0];
        opll->sprev[1] = opll->snext[1];
        calcStereo (opll, opll->snext);
      }

      opll->oplltime -= opll->realstep;
      bufMO[i] = (int32_t) (((double) opll->snext[0] * (opll->opllstep - opll->oplltime)
                           + (double) opll->sprev[0] * opll->oplltime) / opll->opllstep);
      bufRO[i] = (int32_t) (((double) opll->snext[1] * (opll->opllstep - opll->oplltime)
                           + (double) opll->sprev[1] * opll->oplltime) / opll->opllstep);
    }
  }
}
