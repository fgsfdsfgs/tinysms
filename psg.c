...
static PSGCONTEXT* LastChipInit = NULL;

PSGCONTEXT* PSGInit(int clockValue, int samplingRate) {
  int i;
  PSGCONTEXT* chip = (PSGCONTEXT*)malloc(sizeof(PSGCONTEXT));
  if (chip) {
    chip->dClock=(float)(clockValue & 0x7FFFFFF)/16/samplingRate;
    
    PSGSetMute(chip, PSG_MUTE_ALLON);
    PSGConfig(chip, /*MUTE_ALLON,*/ PSG_FEEDBACK, PSG_SRW, 1);
    
    for (i = 0; i <= 3; i++)
      chip->panning[i][0] = chip->panning[i][1] = 1.f;
    //PSGReset(chip);
    
    if ((clockValue & 0x80000000) && LastChipInit != NULL) {
...
void PSGConfig(PSGCONTEXT* chip, /*int mute,*/ int feedback, int sr_width, int boost_noise) {
  //chip->Mute = mute;
...
void PSGUpdate(PSGCONTEXT* chip, int **buffer, int length) {
  ...
  if (NGPMode)
    chip2 = (PSGCONTEXT*)chip->NgpChip2;
